...
def main():
    window = Window("Pendulum", 1200, 800) # Initialize new window with dimensions 1200x800
    pendulum = Pendulum(origin=(600, 250), length = 200, radius=25) # Initialize pendulum at x = 600 y = 250
    window.draw_functions.append(lambda: pendulum.draw(window.screen)) # Appends lambda function for drawing the pendulum to the screen
    window.draw_functions.append(pendulum.update) # Appends pendulum function to updates to variables
